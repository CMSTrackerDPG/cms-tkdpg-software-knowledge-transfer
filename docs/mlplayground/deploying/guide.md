...
2. **Note the `Identifier` of your application.**
...
4. Enter the `Identifier` you noted earlier in the `client_id` field.
...
4. Then, go to `Social applications` and add a new entry, selecting `Cern` as a `Provider` and
   using the `Identifier` and `secret` you noted in the previous section:
